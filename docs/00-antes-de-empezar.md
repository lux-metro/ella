...
> Si nunca antes usaste una Raspberry Pi ni una terminal de Linux, esta guía es para vos. Explicamos qué es cada cosa antes de empezar a conectar cables.
...
Un módulo con un radar de microondas que detecta movimiento (no mide el ambiente sino la gente). Cuando alguien se acerca, la Raspberry Pi lo nota y acelera el tempo de la pieza: la instalación reacciona a los visitantes.
...
| **firmware** | El programa que corre en la ESP32. A diferencia de un archivo común, se "graba" directamente en el chip. |
| **pin** | Los conectores metálicos en los bordes de la ESP32 y la Raspberry Pi. Cada uno tiene un nombre o número. |
...
Preparar la Raspberry Pi → docs/01-raspberry-pi-setup.md
    ↓
Configurar audio → docs/02-audio-setup.md
    ↓
Conectar los cables → docs/03-wiring.md
    ↓
¿Algo no funciona? → docs/04-troubleshooting.md
```

No necesitás tener todo el hardware antes de empezar. Podés arrancar solo con la Raspberry Pi y probar el audio, y agregar el radar de presencia después.
